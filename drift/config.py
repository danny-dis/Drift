...
def load_config() -> dict:
    ...
    config["provider"] = os.environ.get("DRIFT_PROVIDER") or config.get(
        "provider", "openai"
    )
    ...
    # Model
    config["model"] = os.environ.get("DRIFT_MODEL") or config.get(
        "model", "gpt-4o"
    )
    ...
    if provider == "custom" and not config.get("base_url"):
        raise ValueError(
            "Provider 'custom' requires base_url in config.yaml or DRIFT_BASE_URL env var"
        )
