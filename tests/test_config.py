...
def test_default_provider_is_openai(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("drift.config.CONFIG_PATH", str(config_file))
...
def test_env_var_overrides(tmp_path, monkeypatch):
    """DRIFT_PROVIDER and DRIFT_BASE_URL should override config."""
    config_file = tmp_path / "config.yaml"
    config_file.write_text("provider: openai\nmodel: gpt-4.1\napi_key: key\n")
    monkeypatch.setattr("drift.config.CONFIG_PATH", str(config_file))
    monkeypatch.setenv("DRIFT_PROVIDER", "custom")
    monkeypatch.setenv("DRIFT_BASE_URL", "http://localhost:11434/v1")
